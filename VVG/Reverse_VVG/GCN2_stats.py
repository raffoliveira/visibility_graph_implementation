...
def sampling_windows_10_beats(signals_V1: dict, signals_II: dict, rr_interval_pos_signals: dict, 
                              rr_interval_pre_signals: dict) -> dict:
...
def get_beats_features(signals_V1: dict, signals_II: dict, rr_interval_pos_signals: dict, rr_interval_pre_signals:dict) -> dict:
...
def convert_beats_in_graphs(signals_V1: dict, signals_II: dict) -> str:
...
def plotting_acc_loss(acc_values: list, loss_values: list, epochs: int) -> None:
    ...
    plt.savefig(f"./Images2/acc_loss_gcn2_stats.png", dpi=600)
...
def plotting_confusion_matrix(true_label: np.array, pred_label: np.array) -> None:
    ...
    plt.savefig(f"./Images2/confusion_matrix_gcn2_stats.png", dpi=600)
...
def getting_classification_report(true_label: np.array, pred_label: np.array, set_name: str):
    ...
    pond = round(((4584/9316)*n) + ((944/9316)*s) + ((3788/9316)*v), 2)       

    with open(f"./Images2/report_gcn2_stats.txt", "w") as f:
        f.write(set_name)
        f.write("\n")
...
def training(dataset_train: Type[dgl.data.DGLDataset], n_features: int, model_name: str) -> None:
    ...
    model = GCN(n_features, 20, 3)  # (n_nodes_features, n_nodes_hidden_layer, n_classes)
...
if __name__ == '__main__':
    ...
    if MODE == 'Train':
        ...
        train_signals_V1, train_signals_II, train_rr_interval_pos_signals, train_rr_interval_pre_signals = ( 
            segmentation_signals(PATH, FILES_TRAIN, 100, 180, 'Test'))
        ...
        #extraindo os atributos para cada nó (tempo, lead , rr-interval)
        ...
        #convertendo os batimentos em formatos de grafos do DGL
        print("convertendo os batimentos em grafos...")
        train_edges, train_properties = convert_beats_in_graphs(train_signals_V1, train_signals_II)
        
        #criando o dataset com os grafos
        ...
        test_signals_V1, test_signals_II, test_rr_interval_pos_signals, test_rr_interval_pre_signals = (
            segmentation_signals(PATH, FILES_TEST, 100, 180, 'Train'))
